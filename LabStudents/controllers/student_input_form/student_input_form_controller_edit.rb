...
class StudentInputFormControllerEdit
  def initialize(parent_controller, existing_student_id)
    ...
    LoggerClass.instance.debug('StudentInputFormControllerEdit: initialized')
  end

  def set_view(view)
    @view = view
    LoggerClass.instance.debug('StudentInputFormControllerEdit: view set')
  end
  ...
  def process_fields(fields)
    begin
      ...
      LoggerClass.instance.debug('StudentInputFormControllerEdit: replacing student in DB')

      @student_rep.replace_student(@existing_student_id, new_student)

      @view.close
    rescue ArgumentError => e
      LoggerClass.instance.debug("StudentInputFormControllerEdit: wrong fields: #{e.message}")
  ...
  def on_db_conn_error(error)
    LoggerClass.instance.debug('StudentInputFormControllerEdit: DB connection error:')
